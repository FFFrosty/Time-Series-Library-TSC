...
class Transformer_Layer_intralinear(nn.Module):
    def __init__(self, device, d_model, d_ff, num_nodes, patch_nums, patch_size, dynamic, factorized, layer_number, batch_norm):
        super(Transformer_Layer_intralinear, self).__init__()
        self.device = device
        self.d_model = d_model
        self.num_nodes = num_nodes
        self.dynamic = dynamic
...
class Transformer_Layer_interlinear(nn.Module):
    def __init__(self, device, d_model, d_ff, num_nodes, patch_nums, patch_size, dynamic, factorized, layer_number, batch_norm):
        super(Transformer_Layer_interlinear, self).__init__()
        self.device = device
        self.d_model = d_model
        self.num_nodes = num_nodes
        self.dynamic = dynamic
        ...
        d_k = self.inter_d_model // n_heads
        d_v = self.inter_d_model // n_heads
        self.inter_patch_attention = Inter_Patch_Linear(self.inter_d_model, self.inter_d_model, n_heads, d_k, d_v, attn_dropout=0,
                                          proj_dropout=0.1, res_attention=False)
...
class Intra_Patch_Linear(nn.Module):
    def __init__(self, d_model, factorized):
        super(Intra_Patch_Linear, self).__init__()
        self.head = 2
...
class Inter_Patch_Linear(nn.Module):
    def __init__(self, d_model, out_dim, n_heads, d_k=None, d_v=None, res_attention=False, attn_dropout=0.,
                 proj_dropout=0., qkv_bias=True, lsa=False):
        super().__init__()

        # Only Proj output
        self.to_out = nn.Sequential(nn.Linear(d_model, d_model*2),
                                    nn.ReLU(),
    ...
    def forward(self, Q, K=None, V=None, prev=None, key_padding_mask=None, attn_mask=None):
        ...
        output = self.to_out(Q)
        attn_weights = torch.zeros_like(attn_mask) if attn_mask is not None else None
