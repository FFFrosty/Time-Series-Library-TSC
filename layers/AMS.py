...
from layers.Layer import Transformer_Layer_interlinear, Transformer_Layer_intralinear, Transformer_Layer
...
class AMS_intralinear(nn.Module):
    def __init__(self, input_size, output_size, num_experts, device, num_nodes=1, d_model=32, d_ff=64, dynamic=False,
                 patch_size=[8, 6, 4, 2], noisy_gating=True, k=4, layer_number=1, residual_connection=1, batch_norm=False):
        super(AMS_intralinear, self).__init__()
        patch_size = [8, 6, 4, 2]
        self.num_experts = num_experts
        self.output_size = output_size
        self.input_size = input_size
        ...
        # for patch in [patch_size]:
            patch_nums = int(input_size / patch)
            self.experts.append(Transformer_Layer_intralinear(device=device, d_model=d_model, d_ff=d_ff,
                                      dynamic=dynamic, num_nodes=num_nodes, patch_nums=patch_nums,
                                      patch_size=patch, factorized=True, layer_number=layer_number, batch_norm=batch_norm))
...
class AMS_interlinear(nn.Module):
    def __init__(self, input_size, output_size, num_experts, device, num_nodes=1, d_model=32, d_ff=64, dynamic=False,
                 patch_size=[8, 6, 4, 2], noisy_gating=True, k=4, layer_number=1, residual_connection=1, batch_norm=False):
        super(AMS_interlinear, self).__init__()
        patch_size = [8, 6, 4, 2]
        self.num_experts = num_experts
        self.output_size = output_size
        self.input_size = input_size
        ...
        # for patch in [patch_size]:
            patch_nums = int(input_size / patch)
            self.experts.append(Transformer_Layer_interlinear(device=device, d_model=d_model, d_ff=d_ff,
                                      dynamic=dynamic, num_nodes=num_nodes, patch_nums=patch_nums,
                                      patch_size=patch, factorized=True, layer_number=layer_number, batch_norm=batch_norm))
